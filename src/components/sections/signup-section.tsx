...
export function SignupSection() {
   ...
   const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (formData.password.length < 6) {
         setRes("Password must be at least 6 characters");
         setLoading(false);
         return;
      }
      ...
      try {
         ...
         setRes("Signup successful");
         router.push("/login");
         console.log(data);
      } catch (error: any) {
         setRes(error.response?.data?.message || "Signup failed");
      } finally {
         setLoading(false);
      }
   };
         ...
         <div className="container mx-auto px-4 md:px-6">
            <div className="max-w-[1000px] mx-auto w-full flex flex-col md:flex-row gap-6">
               ...
               <div className="flex-1 bg-white rounded-[2rem] shadow-[0_8px_40px_rgb(0,0,0,0.06)] p-8 md:p-14 flex flex-col items-center justify-center border border-gray-100">
                  ...
                  <h1 className="text-[26px] md:text-[28px] font-bold text-[#03112E] mb-3 text-center">Create Your Account</h1>
                  <p className="text-[#4B5563] text-sm md:text-[15px] font-medium text-center mb-10 max-w-[320px]">
                     Sign up to manage your health and stay connected.
                  </p>
                  ...
                  <form onSubmit={handleSubmit} className="w-full max-w-[360px]">
                     <div className="space-y-5 mb-3">
                        ...
                        <div className="space-y-2">
                           <label className="block text-[13px] font-bold text-[#4B5563]">Full Name</label>
                           <input
                              type="text"
                              name="name"
                              value={formData.name}
                     ...
                     {/* Sign Up Button */}
                     <button disabled={loading} type="submit" className="w-full h-12 rounded-full bg-[#03112E] hover:bg-[#0a1f4a] text-white font-bold text-[15px] mb-8 mt-5 transition-colors shadow-md">
                     ...
                     <div className="relative flex items-center justify-center mb-6">
                        ...
                        <div className="relative bg-white px-4 text-[13px] text-gray-400 font-medium">Or Sign up with</div>
                     </div>
                     ...
                     <p className="text-center text-[13px] font-semibold text-gray-500">
                        Already have an account? <Link href="/login" className="text-[#6452F9] hover:underline ml-1">Sign in</Link>
                     </p>
                  </form>
               </div>
